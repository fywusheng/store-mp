<template>
  ...
  >
    <view class="fixHead" :class="fixTop ? '' : 'noshow'">
      <view class="shop" @click="point(1)" :class="actived == 1 ? 'aclass' : ''"
        >商品</view
      >
      <view
        class="detail"
        @click="point(2)"
        :class="actived == 2 ? 'aclass' : ''"
        >详情</view
      >
    </view>
    <view class="empty-wrap" v-if="product && product.saleState !== 5">
      <img
        class="tip"
        src="https://ggllstatic.hpgjzlinfo.com/static/images/item-detail/tip.png"
      />
      <view class="title">很抱歉，您查看的商品已无效！</view>
      <view class="desc">您可以选择浏览其他商品</view>
      <button type="button" class="btn-view" @click="toHome">
        看看其他商品
      </button>
    </view>
    ...
    <template v-else>
      ...
      <view class="pointer"
        >{{ currentIndex }}/{{ productImgList.length }}</view
      >
      <view class="area">
        <view
          class="product-info"
          :class="product.creditPoints ? 'margTop1' : ''"
        >
              ...
              <view class="pres">
                ...
                <view v-else>
                  <span v-if="couponList[0].type == 0"
                    >{{
                      `满${couponList[0].thresholdValue}减` +
                      couponList[0].denominationStr
                    }}元</span
                  >
                  <span v-else-if="couponList[0].type == 1"
                    >{{
                      `满${couponList[0].thresholdValue}打` +
                      couponList[0].denominationStr
                    }}折</span
                  >
                </view>
              ...
              >
                ...
                <view v-else>
                  <span v-if="couponList[1].type == 0"
                    >{{
                      `满${couponList[1].thresholdValue}减` +
                      couponList[1].denominationStr
                    }}元</span
                  >
                  <span v-else-if="couponList[1].type == 1"
                    >{{
                      `满${couponList[1].thresholdValue}打` +
                      couponList[1].denominationStr
                    }}折</span
                  >
                </view>
          ...
          <view
            class="getPrice"
            v-if="product.creditPoints && selectSize.presentPrice >= 0"
          >
            <img
              class="collected"
              src="https://ggllstatic.hpgjzlinfo.com/static/home/getP.png"
            />
          ...
          <view class="wuliu-info">
            ...
            <!-- 全免包邮 -->
            <text
              v-if="freight.isPostage === 1 && freight.postage === 0"
              class="font_value"
              >商家包邮
            </text>
            <!-- 满X元包邮 -->
          ...
          <view class="wuliu-info flex">
            ...
            <text class="text-over">{{
              attributeList[0].name + attributeList[0].value
            }}</text>
            <image
              class="icon-right"
              @click="toAttribute"
        ...
        <view class="_center">
          ...
          <view class="right">
            <view class="_n">
              ...
              <view class="k" v-if="storeList[0].distance"
                >距您{{ storeList[0].distance }}km</view
              >
            </view>
            <view class="_a">{{ storeList[0].storesAddress }}</view>
          </view>
        </view>
        <button class="btn" v-if="storeList.length > 1" @click="watchMore">
          查看更多
        </button>
      </view>
      <view class="line"></view>
      <view class="in-store">
        <view class="left">
          <img
            mode="scaleToFill"
            class="_img"
            :src="product.supplierDTO.supplierStorePicUrl"
          />
          <view class="_right">
            <view class="name">{{
              product.supplierDTO.supplierStoreName
            }}</view>
      ...
      <view class="footer">
        ...
        <view class="btn-buy" @click="checkout">立即购买</view>
      </view>
    </template>
    <select-sku
      ref="selectSku"
  ...
  methods: {
    ...
    toCart() {
      ...
      if (this.sceneType === "积分兑换") {
        ...
        uni.navigateTo({
          url:
            "/sub-pages/index/index/main?index=4&&sceneType" + this.sceneType,
        });
      }
    },
    async changeFavor() {
    ...
    goStoreDetail() {
      ...
      uni.navigateTo({
        url:
          "/sub-pages/index/store/main?supplierId=" + this.product.supplierId,
      });
    },
    watchMore() {
      // 门店
