...
export default {
      ...
      query
        ...
        .boundingClientRect((data) => {
          ...
          if (this.direction == "vertical") {
            this.handleY =
              (this.progressBarInfo.height * (100 - this.percent)) / 100;
          } else {
  ...
  computed: {
    ...
    maxValue() {
      const max = Number.isNaN(parseFloat(this.max))
        ? 100
        : parseFloat(this.max);
  ...
  methods: {
    ...
    textLength(t) {
      ...
      const l =
        ((int.length - subl + this.getStepInfo()[1]) / 3) * 2 + subl + 0.2;
    ...
    handleMove(x_) {
      ...
      if (this.direction == "vertical") {
        ...
        cp = cp > 0 ? cp : 0;
        const value =
          Number((1 - cp) * (this.maxValue - this.minValue)) + this.minValue;
        ...
        cp = cp > 0 ? cp : 0;
        const value =
          Number(cp * (this.maxValue - this.minValue)) + this.minValue;
        this.percent = cp * 100;
        this.showValue = this.valueFormat(value);
        this.handleX = x;
      }
