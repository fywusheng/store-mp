<template>
  <view class="login">
    ...
    <view class="row flex-h flex-c-s m-0-60 b-b-line">
      ...
      <input :auto-blur="true" @focus="addFocus" :adjust-position="false"
        class="row__input fs-40 c-black flex-1" v-model="params.account"
        :type="loginType === 'smsCode' ? 'number' : 'idcard'"
        :maxlength="loginType === 'smsCode' ? 11 : 18" :placeholder="placeholder"
        placeholder-class="placeholder" />
    </view>
    <view class="row flex-h flex-c-s m-0-60 b-b-line" v-if="loginType === 'smsCode'">
      <!-- <text class="row__label fs-40 c-black">验证码：</text> -->
      <input @focus="addFocus" @blur="reduceFocus" class="row__input fs-40 c-black flex-1"
        v-model="params.smsCode" type="number" :adjust-position="false" maxlength="6"
        placeholder="请输入验证码" placeholder-class="placeholder" />
      <button class="row__button fs-36 c-code" hover-class="none" :class="{ 'c-grey': seconds > 0 }"
        :disabled="seconds > 0" @click="handleSencSMSCodeClick">
    ...
    <view class="row flex-h flex-c-s m-0-60 b-b-line" v-else-if="loginType === 'password'">
      ...
      <input @focus="addFocus" @blur="reduceFocus" class="row__input fs-40 c-black flex-1"
        v-model="params.password" type="text" maxlength="8" :adjust-position="false"
        :password="!showsPasswordText" placeholder="请输入密码" placeholder-class="placeholder" />
      <image class="row__icon" mode="scaleToFill" :src="passwordIconURL"
    ...
    <button class="button button-2 fs-44 c-primary m-32" hover-class="none"
    ...
    <view class="footer">
      <view class="radio">
        <image @click="handleCheckXieyi" :class="checked?'icon-check':'icon-noCheck'"
    ...
    <!--人脸识别未通过提示 -->
    <modal ref="tipModal" cancelText='不同意' confirmText='同意' @cancel='handleCancel'
      ...
      <view slot="text">
        <view class="confirm-main" style="height:auto;line-height:1.5">
          <view class="content">
            ...
            和<text class="c-code" @click="handlePrivacyPolicyClick">《隐私协议》</text>的全部条款，
            接受后可开始使用我们的服务
          </view>
        </view>
      </view>
...
export default {
  ...
  computed: {
    ...
    // 密码图标地址
    passwordIconURL() {
      return this.showsPasswordText ? 'https://ggllstatic.hpgjzlinfo.com/static/user-center/icon-user-center-show-password.png' : 'https://ggllstatic.hpgjzlinfo.com/static/user-center/icon-user-center-hide-password.png'
    }
  },
  onLoad(e) {
    if (e.goUrl) {
