    ...
    <view class="page-header mt-16">
      ...
      <view class="market-card bg-white br-24">
        ...
        <view class="flex-h address" @click="handleDirectionClick">
          ...
          <template v-if="loading == 2">
            ...
            <view class="distance"
              >距您{{ setDistance(storeList[0].distance) }}</view
            >
          </template>
      ...
      <view class="discount">
        <image
          ...
          mode="scaleToFill"
        />
        <view class="count">{{ markrtDiscount(info.showDiscount) }}折</view>
      </view>
    </view>
    <view
...
export default {
  ...
  methods: {
    ...
    handleOtherMarket() {
      ...
      uni.navigateTo({
        url:
          "/pages/supermarket/other-market?info=" +
    ...
    handleShowPayCodePage() {
      api.getPayCodePage({
        ...
        success: (data) => {
          uni.navigateTo({
            url:
              "/pages/pay/show-pay-code?url=" + encodeURIComponent(data.result),
          });
        },
      });
    },
