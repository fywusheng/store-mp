    ...
    <view class="gg">
      ...
      <view class="shop_list">
        <scroll-view v-if="itemList.length || loading" scroll-y class="item-list-wrap"
          ...
          <ul class="item-list">
            <li class="item" @click="goItem(item)" v-for="(item,subIndex) in itemList"
              :key="subIndex">
              ...
              <div class="item-price">
                <view class="jf" v-if="item.isCreditPoints == 1">
                  积分抵扣￥{{(item.creditPoints || item.creditPoints==0)?item.creditPoints:''}}
                </view>
...
export default {
  ...
  methods: {
    ...
    clickItem(item) {
      ...
      const urls = { 1: '/pages/user-center/message-center', 2: '/pages/index/index', 3: '/pages/pay/my-bank-card?back=1' }
    ...
    async loadData() {
      ...
      Object.assign(params, this.storeMap.storeIds.length > 0 ? { ids: this.storeMap.storeIds } : {})
      if (this.sortType) {
      ...
      const searchResult2 = await Axios.post('/product/getProductSearchList', Object.assign(params, this.searchParams))
