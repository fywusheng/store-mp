		...
		<navigation-bar :alpha="1">
			<view slot="title1">
				<view class="navigation-bar flex-h flex-c-s" :style="{ height: '44px' }">
					<image class="icon-desc" @click="handleHomeBack"
						src="http://192.168.1.187:10088/static/supermarket/icon-index.png" mode="scaleToFill" />
	...
	export default {
		...
		methods: {
			...
			async loadData() {
				this.loading = true;
				uni.showLoading();
				const result = await Axios.post('/order/get', {
					orderId: this.id
				});
				uni.hideLoading();
				this.loading = false;
			...
			async checkNeedPopUp(popUpType) {
				...
				const {
					code,
					data
				} = await Axios.post('/order/checkNeedPopUp', params);
				if (code === '200') {
					if (data.needPopUp === 1) {
