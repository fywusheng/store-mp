  ...
  <div class="page-service-type">
    <ul class="type-list">
      ...
      <!-- <li class="type" @click="toRefund(3)">
        ...
        <img class="icon-right" src="https://ggllstatic.hpgjzlinfo.com/static/images/common/icon-right.png">
      </li> -->
    </ul>
  </div>
</template>
...
export default {
  ...
  methods: {
    toRefund(type) {
      const url = `../refund-apply/main?type=${type}&itemId=${this.$scope.options.itemId}&num=${this.$scope.options.num}&orderId=${this.$scope.options.orderId}&skuId=${this.$scope.options.skuId}&productId=${this.$scope.options.productId}`;
      uni.redirectTo({
        url: url,
      });
    },
