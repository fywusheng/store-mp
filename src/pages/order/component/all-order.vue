  ...
  <view class="pages">
    <view class="nearby-list" v-if="list.length > 0">
      <view v-for="(item, index) in list" :key="index">
        <!-- 附近优惠 -->
        <view
          v-if="![4, 5, 6, 9, 13].includes(item.orderSource)"
          class="item"
        ...
        >
          ...
          <view class="opr">
            <button
              ...
              v-if="
                item.orderStatus === '1' &&
                checkExpirationTime(item.expirationTime)
              "
            >
              立即支付
            </button>
        ...
        <trip-order-item
          ...
          :item="item.orderDetailDTO"
          :key="item.orderId"
        />
        <!-- 消费养老 -->
        <view class="consum-item" v-if="item.orderSource === 5">
          <consum-order-item
...
import consumOrderItem from "./consum-order-item.vue";
export default {
  components: {
    Modal,
    ShopOrderItem,
