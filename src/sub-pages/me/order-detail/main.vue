  ...
  <div class="page-order-info" v-if="!loading">
    ...
    <div class="order-header-status">
      <div class="status">
        ...
        <div v-if="order.orderStatus == 10">订单将于<span
            style="color:#FF5500">{{minutes}}分{{seconds}}秒</span>后被取消，请尽快支付！</div>
        <div v-if="order.orderStatus == 20">我们会尽快安排发货，请您耐心等待哦！</div>
        <div v-if="order.orderStatus == 30">
          请确认收货，系统将于<span style="color:#FF5500">&nbsp;{{hours}}小时{{minutes}}分</span>后自动确认</div>
    ...
    <div class="receive-info">
      <div class="info-item">
        ...
        <div class="info">
          <div class="name">{{order.receiveName}}<span class="phone">{{order.receivePhone}}</span>
          </div>
          <div class="address">{{order.receiveProvinceName + order.receiveCityName + order.receiveAreaName +
    ...
    <div class="store-list" v-for="(orderItemModel,index) in order.storeOrderItems" :key="index">
      ...
      <ul class="item-list">
        <li class="item" @click="toItem(item)" :key="itemIndex"
          ...
          <div class="sku-name">{{item.skuName}}
            <div class="item-state"
              ...
              <button v-if="item.itemStatus == 1 && !order.hzhH5" type="button" class="btn-link"
                @click.stop="toService(order, item)">
                申请售后
              </button>
              <div v-else @click.stop="toService(order, item)" class="item-state-label">
                {{item.itemStateStr}}</div>
            </div>
          </div>
          <div class="item-price">¥{{item.sellingPrice|formatNum}}
    ...
    <div class="info-list">
      ...
      <div class="info" v-if="order.completionTime">完成时间: <span
          class="value">{{order.completionTime}}</span></div>
    </div>
    <div class="line"></div>
    <div class="cost-list">
      <div class="cost">订单总额: <span class="value">¥{{order.orderAmount|formatNum}}</span></div>
      <div class="cost">运费: <span class="value red">+¥{{order.freightAmount|formatNum}}</span></div>
      <div class="cost">积分抵扣: <span class="value red">-¥{{order.pointsPrice|formatNum}}</span>
      </div>
      <div class="cost">优惠券: <span class="value red">-¥{{order.couponAmount|formatNum}}</span></div>
      <div class="cost">
        <text class="right">实付款:</text> <span
          class="value red">¥{{order.payableAmount|formatNum}}</span>
      </div>
      <!-- <div class="cost">积分:
    ...
    <template v-if="!isMulti">
      ...
      <div class="info-list">
        ...
        <div class="info">{{order.storeOrderItems && order.storeOrderItems[0].receiveRemark || '无'}}
        </div>
      </div>
    </template>
    <div class="line"></div>
    ...
    <div class="line"></div>
    <ul v-if="[10,20,30].includes(order.orderStatus)" class="footer-btn-list"
